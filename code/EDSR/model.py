...
class EDSR(object):
	...
	def __init__(self,img_size=32,num_layers=32,feature_size=256,scale=2,output_channels=3,test=True):
		print("Building EDSR...")
		self.img_size = img_size
		self.scale = scale
		self.target_size = img_size * scale
	...
	"""
	def set_data_fn(self,fn,args,test_set_args=None):
		self.data = fn
		self.args = args
	...
	def train(self,iterations=1000,save_dir="saved_models"):
		...
		with self.sess as sess:
			...
			if self.test:
				...
				testGene = self.data(*self.test_args)
				test_x,test_y = next(testGene)
